...
The following code is divided into 4 sections:
-Network
-Outside connections 
-Topic modelling
-Not important (test code for graphs copied into the shiny server, random calculations, failed attempts, etc.)

SECTION 1: NETWORK

```{r create raw network}
#The following section is dedicated to creating the raw network.

#It includes the following:
#clean_to_network_cols function, which does very basic cleaning and selects the needed columns
#change_link_to_name function, which converts link format to share format for the channel data
#count_connections function, which counts the total connections (combined links and shares) to each other channel for each source channel. 
#create_connections function, which reads the data and combines the three above functions.
#Performs create_connections() on all my raw data.
#Creates the tibble big_network, which binds the rows of all the connections data. big_network shows each source channel, each channel it connects to, and how many times.
...
saveRDS(big_network, file = "belarus-protests/clean_data/big_network.RDS")
...
#The following section adds to the raw big network created above through outside information and some calculations.
#The output is a tibble corpus_network_info that includes the following information about the data that I scraped: total links/shares to and from channel, unique (each channel counted just once) links/shares to and from each channel, the difference between those (whether I classify the channel as a source or hub), subscribers, url and a few other data points added from the excel document I created called "telegram_corpus." 
...
#Reads the telegram_corpus information from excel
...
#Combines above tibbles into one tibble and adds a column about whether the hub and source classifications agree based on each counting method.
...
saveRDS(corpus_network_info, "belarus-protests/clean_data/corpus_network_info.RDS")
  
```


```{r small network graph}
#this section is for making the small network (just channels I scraped data from) graph. The output is edges and nodes, formatted as necessary to create an igraph object, which is done in the server. 
...
saveRDS(small_nodes, "belarus-protests/clean_data/small_nodes.RDS")
...
saveRDS(small_edges, "belarus-protests/clean_data/small_edges.RDS")

```

```{r big network graph}

#This section is about creating the big network graph. It is virtually the same as above except for all the nodes and edges, not just the ones within the leading channel corpus. 
...
saveRDS(big_nodes, "belarus-protests/clean_data/big_nodes.RDS")
...
saveRDS(big_edges, "belarus-protests/clean_data/big_edges.RDS")

```
SECTION 2: OUTSIDE CONNECTIONS

```{r, warning = FALSE}
#This section is about cleaning and graphing connections to non-telegram sources. The output is:
#A function, create_outside_connections, that reads the data, shortens non-Telegram urls to just the domain name, and counts how many times each domain appears for each channel.
#Performs create_outside_connections on all my data.
#Combines the above into a single tibble, big_outside_network.
...
saveRDS(big_outside_network, "belarus-protests/clean_data/big_outside_network.RDS")
...
#This section is about cleaning the text data into a tidytext format. The output is:
#A function, clean_text, that reads the raw data and cleans it to a one-word-per-row format with the source, message ID and date/time. The words are stemmed and stopwords/punctuation removed.
#A function, create_tokenized, which performs clean_text and then groups by message and creates a list column with each stemmed word in the message (this turned out to be useless because I did not end up using data in this format and later unnested it, but it would have been a hassle to redo the intermediary code so I left it.)
#A function, count_words, which counts the number of times each word occurs in each source. 
...
clean_text <- function(data, name){
  ...
  #there were two possible locations for message text, this combines them into one column.
  mutate(text_raw = ifelse(is.na(text1), text2, text1)) %>%
  select(id, date, text_raw) %>%
  filter(!is.na(text_raw)) %>%
  #This nice line of code made sure each word had the date/time and message ID by pulling those down from the most recent one, since only the first word in each message had it previously.
...
#Counts the number of times the word occurs in each source
count_words <- function(data){
  data %>%
  group_by(source, tokenized_words) %>%
  summarize(count = n(), .groups = "drop") %>%
  arrange(desc(count))
}
  
  
  #This was a bit of code I tried to use to remove emojis. It did not work. 
...
#In this section I perform the text cleaning. The output is a tibble, tokenized_texts, that is the result of binding rows of all the create_tokenized tibbles from the raw data, unnesting the list columns, and using count_words to count the number of times each word occurs in each source. 
#This was the format that the package topicmodels required. I followed instructions here: https://www.tidytextmining.com/nasa.html
...
nexta_live_tokenized <- create_tokenized("raw_data_csv/nexta_live.csv", "NEXTA Live")
  ...
  #But I have to remove the ones that are (primarily) Belarussian :(
  ...
  tokenized_texts <- bind_rows(nexta_live_tokenized, 
    ...
    #First I unnest the tokenized words, this makes it into a tidytext format
    
    unnest(tokenized_words) %>%
    
    #Next I have to count the words grouped by source. This treats the whole corpus from a channel as one text. 
...
```{r topicmodels}
#This section is about performing the topic modeling. The output is two tibbles.
#The first, top_20, includes the top 20 words for each topic.
#The second, gamma, includes the percentage of each channel that is each topic

#I'm continuing to follow instructions from here: https://www.tidytextmining.com/nasa.html

#After running the topic models there are some words that would be good to manually remove because they are not meaningful.

my_stop_words <- tibble(tokenized_words = c("—",
                                            "–",
                                            "эт",
                                            "котор",
                                            "наш",
...
#anti_join removes columns in which the tokenized_word column matches for my_stop_words and tokenized_texts. 

tokenized_texts2 <- anti_join(tokenized_texts, my_stop_words, by = "tokenized_words")

#To use the topicmodels package I need to turn my tidytext format into a document term matrix using cast_dtm from the tidytext package.

dtm <- tokenized_texts2 %>% 
  cast_dtm(source, tokenized_words, count)
...
#k is the number of topics. There is no a priori reason to know how many topics would work well. I tried many options 1-10 and found 3 to be most meaninful / distinct. 

desc_lda <- LDA(dtm, k = 3, control = list(seed = 1234))
...
#After I was satisfied with the topic model I translated each word in a tibble with a single column, english. 
english_translation <- tibble(english = c(#topic 1
                                          "deputy",
                                          "issue/question (vopros)",
                                          "people",
                                          "need",
                                          "simple",
                                          "chat",
...
#This just smashed my English translation onto the top_20 tibble. This was dangerous, but the list was only 60 words so I manually checked that it was correct (none were out of order or missing)
top_20 <- bind_cols(top_20, english_translation)

saveRDS(top_20, file = "belarus-protests/clean_data/top_20.RDS")


#Now I am following instructions here https://cfss.uchicago.edu/notes/topic-modeling/ to figure out what percentage of each topic is in each document

#Using matrix = "gamma" from the tidytext package apparently grabs the info on percent topic for each document from the LDA topic model. 

gamma <- tidy(desc_lda, matrix = "gamma") %>%
  #Had to rename so that when I join below, the by argument can be the same for both datasets that I am joining. 
  
  rename(channel = document) %>%
  
  #I had to add the telegram_corpus so that I could match urls to the channel name for English speakers. 
  #Was important to do a left_join because there were some channels in the telegram_corpus that were not in the topic model (the Belarussian language channels) and I wanted to make sure all the data from the topic model was preserved. 
  
  left_join(telegram_corpus, by = "channel")

saveRDS(gamma, "belarus-protests/clean_data/gamma.RDS")

```



SECTION 4: NOT IMPORTANT

```{r model about subscribers}
#the following model is included in the server. It is a model seeing if there is a correlation between position in the network (links to and from channel by each counting method) and the number of subscribers. 
...
```{r subscriber plot}
#This plot, which shows the channels by subscriber and type, is reproduced (with some changes- I ended up using my qualitative classification for fill rather than the calculated source/hub distinction), in the server.  
...
```{r relationship between links and subscribers plot}
#This plot is reproduced in the server with interactivity. 
...
```{r plot about most connected channels}
#I did not end up using this plot. 
...
```{r small network unused or replicated}
#This chunk contains network visualizations either not used or replicated in the server.


#This creates the graph object, which I did in the server. D is the edges, and vertices argument is for the nodes.

small_network_graph <- graph_from_data_frame(d = small_edges, directed = T, vertices = small_nodes)

#I ended up not using the following attempts with igraph or calculated graph statistics
...
#Plot 1: simple graph with arrows and labels (not in project)
...
#Plot 2: graph without arrows, with source/hub info and subscribers (something similar with improvments in the server)
ggraph(small_network_graph, layout = "lgl") +
...
# adding network statistics that could be useful in plotting (did not end up using these)
...
#I betweenness is a directed or undirected measure but no "in" or "out" distinction, also I added the weights from the edges. 
...
#Graph 3: betweenness and indegree (did not end up using because betweenness was confusing to me)
ggraph(small_network_graph, layout = "lgl") +
  geom_edge_fan(aes(alpha = sqrt(weight))) +
...
```{r big network graph unused or replicated}
#This section contains big network graph information that was unused or replicated in the server.

#Creating the graph object, which I did in the server
big_network_graph <- graph_from_data_frame(d = big_edges, directed = F, vertices = big_nodes)

#Did not end up using these graph statistics
...
#Improved version of this is in the server
ggraph(big_network_graph, layout = "lgl") +
  geom_edge_fan(aes(alpha = sqrt(weight))) +
  theme_void() +
  #Adds label only if it is in the corpus, otherwise blank
...
```{r random calculations}
#some useful numbers about density of network I wanted to calculate for my paper (did not show up on the website)

#mean unique connections for channels in the corpus
small_edges %>%
  group_by(from) %>%
  summarize(unique_connections = n(), .groups = "drop") %>%
  summarize(avg = mean(unique_connections))

#mean total connections for channels in the corpus
small_edges %>%
  group_by(from) %>%
  summarize(avg_links_per_channel = mean(weight), .groups = "drop") %>%
  summarize(avg = mean(avg_links_per_channel))

#Number of connections outside the corpus
...
#Average connections outside the corpus for the neighborhood groups
big_network %>%
  mutate(in_corpus = ifelse(network %in% small_nodes$label, TRUE, FALSE)) %>%
  filter(in_corpus == FALSE) %>%
...
#Average connections outside the corpus for the non-neighborhood groups
big_network %>%
  mutate(in_corpus = ifelse(network %in% small_nodes$label, TRUE, FALSE)) %>%
  filter(in_corpus == FALSE) %>%
  filter(!(source %in% c("Neighborhood Example 1", "Neighborhood Example 2", "Neighborhood Example 3"))) %>%
...
```{r graph of outside connections}

#The following graph was replicated with improvements in the server.
big_outside_network %>%
  group_by(domain) %>%
  summarize(total = n(), 
            .groups = "drop") %>%
...
```{r attempts to use keyATM}
#The topic modelling package keyATM did not work for me because it refused to accept my data. I could not resolve the error so I used a different package. The code below was used to transform tokenized_texts into a format that keyATM said it needed (instead of a list column, all the words separated by a single space). I could not get my data in a format that it found acceptable, and couldn't follow their exact data processing step recommendations in quanteda because it would not work for my Russian/Belarussian text. 
...
```{r topic model graphs}
#These graphs are in the server.
...
top_20 %>%
  ...
  labs(x = "Percent of topic comprised of this word",
       y = "") +
  scale_color_manual(values = c("#93032E", "#C69F89", "#034C3C"))
