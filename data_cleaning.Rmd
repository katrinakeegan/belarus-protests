...
nexta_text_clean <- raw_nexta %>%
  ...
  #I tried gsub("\n", "", text_raw) as well as gsub("[\r\n]") and the UTF-8 encoding "U+000A."
  
  mutate(text_no_n = gsub("U+000A", "", text_raw)) %>%
  #mutate(text_no_n = str_replace(text_raw, pattern = "\n")) %>%
  mutate(text_no_num = removeNumbers(text_no_n)) %>%
  mutate(text_no_punc = removePunctuation(text_no_num)) %>%
  mutate(text_list = map(text_no_punc, ~str_split(., pattern = " "))) %>%
  
  #Can't get it to unnest so that each word has own row
  pmap_dfr(data.frame)
```
