...
#It would be good if the nodes here could also have the @name to be more English-friendly. 
...
#I think my from and to might be reversed?

plot.igraph(small_network_graph,
...
ggraph(small_network_graph, layout = "lgl") +
  geom_edge_fan(aes(alpha = log(weight))) +
  geom_node_text(aes(label = V(small_network_graph)$label),
                 size = 2,
                 color = "red")
...
clean_text <- function(data, name){
  ...
  filter(!word_raw %in% stopwords(kind = "ru")) %>%
